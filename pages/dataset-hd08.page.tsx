import * as datasetHd08 from "dataset-hd08"
import type { HighDensityRepair01Input } from "lib/types"
import { useEffect, useState } from "react"
import { Debugger } from "./components/Debugger"

const datasetModule = datasetHd08 as Record<string, unknown>

const sampleNames = Object.keys(datasetModule)
  .filter((sampleName) => /^sample\d{4}$/.test(sampleName))
  .sort()
...
const clampSampleIndex = (sampleIndex: number) =>
  Math.min(
    ...
    Math.max(sampleNames.length - 1, 0),
  )
...
export default function DatasetHd08Page() {
  ...
  if (sampleNames.length === 0) {
    return (
      <div className="flex h-screen items-center justify-center p-6">
        No `dataset-hd08` samples found.
      </div>
    )
  }

  const selectedSampleName = sampleNames[selectedSampleIndex] ?? sampleNames[0]!
  const selectedSample = datasetModule[
    selectedSampleName
  ] as HighDensityRepair01Input
  ...
  return (
    <div className="flex h-screen flex-col gap-3 p-3">
      <div className="flex flex-wrap items-center gap-3 rounded border border-slate-300 bg-white p-3">
        ...
        <button
          className="rounded border border-slate-300 px-3 py-1 text-sm disabled:opacity-50"
          disabled={selectedSampleIndex === sampleNames.length - 1}
          onClick={() =>
            setSelectedSampleIndex((index) =>
              Math.min(index + 1, sampleNames.length - 1),
            )
          }
          type="button"
        >
        ...
        <div className="text-sm text-slate-700">
          {selectedSampleName} • sample {selectedSampleIndex + 1} of{" "}
          {sampleNames.length}
          {" • "}
          {selectedSample.nodeWithPortPoints.portPoints.length} port points
          {" • "}
          {selectedSample.nodeHdRoutes.length} reference routes
        </div>
      </div>
      <div className="min-h-0 flex-1">
        <Debugger key={selectedSampleName} sample={selectedSample} />
      </div>
    </div>
  )
}
